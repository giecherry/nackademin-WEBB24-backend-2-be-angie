import type { Cocktail, CocktailApiResponse, CocktailApiDrink, Ingredient } from "../types/cocktail.js";


export const transformApiDrinkToCocktail = (apiDrink: CocktailApiDrink): Cocktail => {
    const ingredients: Ingredient[] = [];
    
    for (let i = 1; i <= 15; i++) {
        const ingredientKey = `strIngredient${i}` as keyof CocktailApiDrink;
        const measureKey = `strMeasure${i}` as keyof CocktailApiDrink;
...
export const fetchRandomCocktail = async (): Promise<Cocktail | null> => {
